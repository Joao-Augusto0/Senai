...
public class Petform extends JFrame implements ActionListener {
	...
	private String imgIco = "./src//img//panda.jpg";
	private String[] imagens = { "./src//img/cachorro.jpg", "./src/img/gato.jpg", "./src/img/coelho.jpg",
			"./src/img/panda.jpg" };
	...
	private void cadastrar() {
		if (tfNomePet.getText().length() != 0 && tfRaca.getText().length() != 0 && tfPeso.getText().length() != 0
			...
			PetProcessa.pets.add(new Pet(autoId, cbEspecie.getSelectedItem().toString(), tfNomePet.getText(),
	...
	private void buscar() {
		...
		if (isNumeric) {
			...
			if (PetProcessa.pets.contains(pet)) {
				...
				tfId.setText(PetProcessa.pets.get(indice).getId("s"));
				cbEspecie.setSelectedIndex(obterIndiceEspecie(PetProcessa.pets.get(indice).getEspecie()));
				tfNomePet.setText(PetProcessa.pets.get(indice).getNomePet());
				tfRaca.setText(PetProcessa.pets.get(indice).getRaca());
				tfPeso.setText(PetProcessa.pets.get(indice).getPeso("s"));
				tfNascimento.setText(PetProcessa.pets.get(indice).getNascimento("s"));
				tfNomeDono.setText(PetProcessa.pets.get(indice).getNomeDono());
				tfTelefone.setText(PetProcessa.pets.get(indice).getTelefone());
	...
	private void alterar() {
		...
		if (tfNomePet.getText().length() != 0 && tfRaca.getText().length() != 0 && tfPeso.getText().length() != 0
			...
			PetProcessa.pets.set(indice, new Pet(id, cbEspecie.getSelectedItem().toString(), tfNomePet.getText(),
					tfRaca.getText(), peso, tfNascimento.getText(), tfNomeDono.getText(), tfTelefone.getText()));
			preencherAreaDeTexto();
			limparCampos();
		} else {
